module ID_csr_transfer #(
    ...
    input csr_op_t csr_opcode,

    input wire [DATA_WIDTH-1:0] x_rs1,  // from RF
    output wire rs1_o,  // to RF

    output logic [DATA_WIDTH-1:0] csr_data,  // to EXE
    
    // From CSR transfer
    input priviledge_mode_t priviledge_mode_i,
    input mtvec_t mtvec_i,
    input mscratch_t mscratch_i,
    input mepc_t mepc_i,
    input mcause_t mcause_i,
    input mstatus_t mstatus_i,
    input mie_t mie_i,
    input mip_t mip_i,
    input mtval_t mtval_i,
    input mideleg_t mideleg_i,
    input medeleg_t medeleg_i,
    input satp_t satp_i,
    input sepc_t sepc_i,
    input scause_t scause_i,
    input stval_t stval_i,
    input stvec_t stvec_i,
    input sscratch_t sscratch_i,
    ...
    output wire priviledge_mode_wen,
    output wire mtvec_wen,
    output wire mscratch_wen,
    output wire mepc_wen,
    output wire mcause_wen,
    output wire mstatus_wen,
    output wire mie_wen,
    output wire mip_wen,
    output wire mtval_wen,
    output wire mideleg_wen,
    output wire medeleg_wen,
    output wire satp_wen,
    output wire sepc_wen,
    output wire scause_wen,
    output wire stval_wen,
    output wire stvec_wen,
    output wire sscratch_wen
